...
CHECKS FAILING:{% for check in service.all_failing_checks %}
  FAILING - {{ check.name }} - Type: {{ check.check_category }} - Importance: {{ check.get_importance_display }} - 
   ({{ check.last_result.error|safe }})
  # Last Result: {{ check.last_result.raw_data }}
  {% endfor %}
...
def send_hipchat_alert(service, users, duty_officers):
    ...
    _send_hipchat_alert(message, service, color=color, sender='Cabot/%s' % service.name)
